...
namespace Host4Travel.BLL.Validators
{
    public class CategoryValidator:AbstractValidator<Category>
    {
        public CategoryValidator()
        {
            ...
            RuleFor(x => x.CategoryLevel).NotNull().NotEmpty().WithMessage("Kategori seviyesi belirtilmelidir.");
            RuleFor(x => x.CategoryLevel).GreaterThanOrEqualTo(0)
                .WithMessage("Kategori seviyesi 0 veya 0 dan büyük olmalıdır");
            RuleFor(x => x.CategoryName).NotEmpty().NotNull().WithMessage("Kategori ismi belirtilmelidir.");
            ...
            RuleFor(x => x.CategoryUrl).NotNull().NotEmpty().WithMessage("Kategori URL Adresi belirtilmelidir.");
            RuleFor(x => x.IsActive).NotNull().NotEmpty().WithMessage("Kategorinin aktiflik durumu belirtilmelidir");
